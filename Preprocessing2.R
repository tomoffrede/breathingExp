...
listCBr <- listCB[grepl("joint|alone|Baseline", listCB)]
...
listCTr <- listCT[grepl("-Schwalbe|-Hirsch|-Pferd|Baseline", listCT)]
listCTf <- listCT[grepl("Holidays|Hobbies|Home", listCT)]
...
listCWr <- listCW[grepl("joint|alone|Baseline", listCW)]
...
for(i in 1:nrow(listREAD)){
  ...
  if(grepl("Baseline", listREAD$breath[i])){
    act <- "watching"
  }
...
save(brm2, file=paste0(folder, "DataBreathing2.RData"))
save(frb2, file=paste0(folder, "DataReadSpeech2.RData"))
