...
(There was only an effect when we used the z-scored breathing rate, but not the actual values).
...
However, if we look at the duration of breath cycles (negatively related to breathing rate), we see longer cycle during `interaction` than during `baseline` (alone). This is the opposite from what we expected.

```{r}
summary(c1a <- lmer(cycleDur ~ Cond2 + (1  | Speaker), dat))

par(mfrow=c(2, 2))
hist(resid(c1a))
qqnorm(resid(c1a));qqline(resid(c1a))
plot(fitted(c1a), resid(c1a))
```
...
Inhalation amplitude gets larger with physical effort. Why the difference between inhalation duration and amplitude? Maybe different people have different strategies for inhaling when they need more oxygen -- some people may inhale longer while others "stronger"? Or else, the correlation between amplitude and duration may only be true for people sitting (our participants); when doing exercise + speaking and actually needing more oxygen, people may just inhale "stronger" instead of longer. Just speculation...
...
People **don't** change their articulation rate across conditions. With the new speech rate data, we also did not replicate the *interaction vs. baseline* effect.

```{r}
summary(s5a <- lmer(speechRateIPU ~ Condition + (1 | Speaker),  df))
summary(s5a <- lmer(speechRateIPU ~ Cond2 + (1 | Speaker),  df))
...
```{r}
summary(s5a <- lmer(articRateIPU ~ Condition + (1  | Speaker),  df))
...
Speech rate was faster during `sitting` than during `biking`.

```{r}
summary(s1 <- lm(speechRateIPU ~ Condition, df))
summary(s1 <- lm(speechRateIPU ~ Cond3, df))
```
...
```{r}
summary(s1 <- lm(articRateIPU ~ Condition, df))
summary(s1 <- lm(articRateIPU ~ Cond3, df))
```
