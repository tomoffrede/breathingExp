...
summary(m2a <- lmer(breathRate ~ Condition + (1 |Speaker), dat %>% filter(act=="watching")))
```
...
summary(a3 <- lmer(inhalAmp ~ act * Condition + (1 | Speaker), dat))
summary(a4 <- lmer(inhalAmp ~ Condition + (1 | Speaker), dat %>% filter(act=="watching")))
summary(a5 <- lmer(inhalAmp ~ Condition + (1 | Speaker), dat %>% filter(act=="listening")))
...
```{r}
summary(d4 <- lmer(inhalDur ~ numberIPUs + Cond2 + (1 | Speaker), dat))
...
```{r}
summary(a2 <- lmer(inhalAmp ~ Cond2 + numberIPUs + (1 | Speaker), dat))
...
During read speech there's an `alone` vs `interaction` effect on breathing rate. Breathing is faster during the interaction, which makes sense in terms of cognitive effort.

The residuals have a few outliers -- do I just remove outlying data points?

```{r}
summary(b1 <- lmer(breathRate ~ Cond2 + (1 | Speaker), dat))
...
```{r}
summary(a2 <- lmer(inhalAmp ~ Cond2 + (1 | Speaker), dat))
...
summary(s5a <- lmer(articRateIPU ~ Cond2 + (1 | Speaker),  df))
