...
**And this seems to be supported by a regression** (reference value: baseline)
```{r, echo=FALSE}
load(paste0(folder, "DataNoDiff.RData"))
df <- dat[dat$Task=="Free",] %>% filter(Speaker != "Confederate")
summary(lmer(f0mean ~ Condition + (1 | Speaker), data = df)) # adding a random slope for condition per participant makes the model singular! even if we don't do the interaction between the random slope and random intercept
```
...
df <- dat %>%
  filter(Task=="Free") %>%
  filter(f0IPUmean > 125)

ggplot() +
  geom_point(df %>% filter(Role=="Participant"),
    mapping=aes(x = CbreathRateSpeech, y = f0IPUmean), size=4, color="pink") +
  geom_point(df %>% filter(Role=="Confederate"),
    mapping=aes(x = CbreathRateSpeech, y = Cf0IPUmean), size=4, color="black")+
...
load(paste0(folder, "DataWithDiff.RData"))
df <- dat[dat$Task == "Free",]
summary(lmer(f0IPUmean ~ CbreathRateSpeech + (1 | Speaker), data = df))
```
